...
function tx_magelink_ajax_display_block(block, id, div)
{
    jQuery.ajax({
        type: "POST",
        dataType: "html",
        ...
        data: {
            'tx_magelink_blockdisplay[block]' : block
        },
        beforeSend: function()
        {
        },
    ...
    jQuery.ajax({
        ...
        success: function(response){
            if (response.status == 'success')
            {
                ...
                {
                    tx_magelink_ajax_add_flash_message( locallang.translate("response_broken"), response.status, true );
                }
            
            }
            else
            {
                tx_magelink_ajax_add_flash_message(response.message, response.status, true);
            }
...
/* refresh the cart */
function tx_magelink_ajax_refreshcart(cartdata)
{
    var magentoUrl = jQuery("input#magentoUrl").val();
...
function tx_magelink_ajax_getcart()
{
    ...
    jQuery.ajax({
        type: "GET",
        dataType: "jsonp",
        jsonpCallback: 'tx_magelink_ajax_refreshcart',
        jsonp: "jsonp",
        url: url, 
        params:{}, 
        data: {
